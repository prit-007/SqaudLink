...
export class CryptoService {
  ...
  static async initializeDevice(userId: string): Promise<DeviceInfo> {
    ...
    if (error) {
      ...
      if (error.code === '42P01' || error.message.includes('relation "user_devices" does not exist')) {
        console.warn('⚠️ E2EE table not set up. Run migration_v2.1_spice_pack.sql to enable E2EE.')
        console.warn('Messages will be stored in plaintext until E2EE is configured.')
        return {
          id: 'no-e2ee',
          name: deviceName,
          fingerprint,
   ...
   */
  private static async wrapKey(aesKey: CryptoKey, rsaPublicKey: CryptoKey): Promise<string> {
   ...
   */
  static async decryptMessage(payload: EncryptedPayload | string): Promise<string> {
    ...
    if (!encryptedKeyBase64) {
      throw new Error('Message not encrypted for this device. Try syncing on the original device.');
    }
   ...
   */
  private static async generatePreKeys(userId: string, deviceId: string): Promise<void> {
   ...
   */
  static async checkAndReplenishPreKeys(userId: string, deviceId: string): Promise<void> {
   ...
   */
  static async claimPreKey(recipientUserId: string, deviceId?: string): Promise<PreKey | null> {
    const { data, error } = await supabase
   ...
   */
  private static async getActiveDevices(userId: string): Promise<Array<{ id: string; publicKey: string }>> {
   ...
   */
  static async listMyDevices(): Promise<Array<{
    id: string;
    name: string;
    lastActive: string;
    isCurrent: boolean;
   ...
   * Save private key to IndexedDB
   */
  private static async savePrivateKey(privateKey: CryptoKey, deviceId: string): Promise<void> {
